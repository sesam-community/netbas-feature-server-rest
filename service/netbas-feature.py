...
def get_paged_entities(path):
    ...
    request_for_count = requests.get(url_count, auth=HttpNtlmAuth(SYSTEM_USER, SYSTEM_PASSWORD), verify=False)
    ...
    while result_count > result_offset and count == 0:
        ...
        req = requests.get(url, auth=HttpNtlmAuth(SYSTEM_USER, SYSTEM_PASSWORD), verify=False)
        if not req.ok:
